...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileSelect = async (file: File, documentType: string) => {
    // Validate file size
    const maxSizeMB =
      documents.find((d) => d.name === documentType)?.max_size_mb || 10;
    ...
    // Validate file type
    const acceptedTypes =
      documents.find((d) => d.name === documentType)?.accepted_types || [];
  ...
  const handleSubmit = () => {
    const completedUploads = uploadedFiles.filter(
      (uf) => uf.status === "completed"
    );
  ...
  const allRequiredUploaded = documents
    ...
    .every((doc) =>
      uploadedFiles.some(
        (uf) => uf.documentType === doc.name && uf.status === "completed"
      )
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {documents.map((document) => {
          const uploadedFile = uploadedFiles.find(
            (uf) => uf.documentType === document.name
          );
              ...
              <div className="flex items-center gap-4">
                ...
                <div
                  className={`p-3 rounded-lg ${
                    isUploaded ? "bg-gray-200" : "bg-gray-100"
                  }`}
                >
                  {isUploaded ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Accepted: {document.accepted_types.join(", ")} • Max size:{" "}
                  ...
                  {isUploaded && (
                    <div className="mt-3 flex items-center justify-between bg-gray-100 rounded-lg p-3">
                      ...
                      <button
                        onClick={() =>
                          uploadedFile && removeFile(uploadedFile.file)
                        }
                ...
                {!isUploaded && (
                  <div>
                    <input
                      type="file"
                      accept={document.accepted_types
                        .map((type) => `.${type}`)
                        .join(",")}
                      onChange={(e) => {
                    ...
                    <label
                      ...
                      className="cursor-pointer bg-gray-900 hover:bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
                    >
                      <Upload size={16} />
                      Choose File
                    </label>
      ...
      {uploadedFiles.length > 0 && (
        <div className="pt-4">
          ...
          >
            {allRequiredUploaded
              ? `Submit Documents (${
                  uploadedFiles.filter((uf) => uf.status === "completed").length
                } uploaded)`
      ...
      <div className="text-sm text-gray-600 bg-gray-50 p-4 rounded-lg border border-gray-200">
        💡 <strong>Tip:</strong> Take clear photos of your documents or scan
        them for best results. Make sure all text is readable and the entire
        document is visible.
      </div>
    </div>
  );
